...
export function FocusTimer() {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center py-10">
      ...
      {/* Mode Selector */}
      <div className="relative z-10 mb-12 flex rounded-full bg-dark-surface p-1.5 ring-1 ring-dark-border shadow-sm">
        {(['2', '10', '15', '25'] as TimerMode[]).map((m) => (
          <button
            ...
            disabled={isRunning}
            className={`rounded-full px-8 py-3 text-base font-semibold transition-all duration-200 ${
              mode === m
      ...
      {/* Timer Circle */}
      <div className="relative mb-12 flex h-64 w-64 shrink-0 items-center justify-center sm:h-80 sm:w-80 pointer-events-none mx-auto">
        <svg 
      ...
      <div className="flex items-center justify-center gap-6 mt-4">
        {!isRunning ? (
          <Button onClick={start} className="w-52 h-16 rounded-full text-xl shadow-lg shadow-brand-500/20 transition-transform hover:scale-105 active:scale-95 flex items-center justify-center gap-3 !px-0 !py-0">
            <Play className="h-7 w-7 fill-current" /> Start
          </Button>
        ) : (
          <Button onClick={pause} variant="secondary" className="w-52 h-16 rounded-full text-xl border border-dark-border shadow-lg transition-transform hover:scale-105 active:scale-95 flex items-center justify-center gap-3 !px-0 !py-0">
      ...
      {activeTask && (
        <motion.div 
          ...
          className="mt-16 w-full max-w-sm rounded-2xl border border-brand-500/20 bg-brand-500/5 p-5 text-center"
        >
          <p className="text-sm font-medium text-brand-400 mb-1">Currently Focusing On</p>
          <p className="font-semibold text-lg mb-4">{activeTask.title}</p>
          <Button 
            variant="secondary" 
            size="sm" 
